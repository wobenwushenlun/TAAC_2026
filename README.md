...
- `version/v080_light_regularization_on_v033/`：最终保留的最优单模型版本。
...
4. **多 epoch checkpoint 保存**  
   从第 3 个 epoch 开始每个 epoch 保存 checkpoint，以验证集 LogLoss 追踪最优点，并在最低 LogLoss 后继续保存 3 个 epoch 后停止，避免只保留最后一个可能过拟合的模型。
...
python -m py_compile version/v080_light_regularization_on_v033/*.py
```

在官方平台提交时，使用 `version/v080_light_regularization_on_v033/` 下的全部文件替换训练模板文件即可。
