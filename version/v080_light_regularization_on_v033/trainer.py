...
class PCVRHyFormerRankingTrainer:
    ...
    def __init__(
        ...
        save_each_epoch_from: int = 0,
        stop_after_best_logloss_epochs: int = 0,
    ) -> None:
        ...
        self.use_amp: bool = (
            str(device).startswith('cuda') and self.amp_dtype in ('bf16', 'fp16')
        )
        self.amp_torch_dtype = torch.bfloat16 if self.amp_dtype == 'bf16' else torch.float16
        self.use_grad_scaler: bool = self.use_amp and self.amp_dtype == 'fp16'
        try:
        ...
        self.save_each_epoch_from: int = save_each_epoch_from
        self.stop_after_best_logloss_epochs: int = stop_after_best_logloss_epochs
        self.best_epoch_logloss: Optional[float] = None
        self.best_epoch_logloss_epoch: Optional[int] = None
        self.epochs_after_best_logloss: int = 0

        logging.info(f"PCVRHyFormerRankingTrainer loss_type={loss_type}, "
                     f"focal_alpha={focal_alpha}, focal_gamma={focal_gamma}, "
                     f"reinit_sparse_after_epoch={reinit_sparse_after_epoch}, "
                     f"amp_dtype={self.amp_dtype}, use_amp={self.use_amp}, "
                     f"save_each_epoch_from={self.save_each_epoch_from}, "
                     f"stop_after_best_logloss_epochs={self.stop_after_best_logloss_epochs}")

    def _amp_context(self):
        if not self.use_amp:
            return nullcontext()
        return torch.amp.autocast('cuda', dtype=self.amp_torch_dtype)
    ...
    def _build_step_dir_name(
        ...
        ``global_step2500.epoch=3.layer=2.head=4.hidden=64[.best_model]``.
        """
    ...
    def _handle_epoch_logloss_checkpoint(
        self,
        epoch: int,
        total_step: int,
        val_auc: float,
        ...
        """Track epoch-end logloss, keep epoch checkpoints, and signal stop.

        The lowest logloss is tracked from epoch 1 onward. Separate checkpoint
        directories are only written from ``save_each_epoch_from`` onward.
        Training stops after ``stop_after_best_logloss_epochs`` consecutive
        epoch-end validations fail to improve the lowest seen logloss.
        """
        is_new_best_logloss = (
            self.best_epoch_logloss is None
            or val_logloss < self.best_epoch_logloss
        )
        if is_new_best_logloss:
            self.best_epoch_logloss = val_logloss
            self.best_epoch_logloss_epoch = epoch
            self.epochs_after_best_logloss = 0
            logging.info(
                f"Best validation LogLoss updated at epoch {epoch}: "
                f"{val_logloss}"
            )
        else:
            self.epochs_after_best_logloss += 1
            logging.info(
                "Validation LogLoss did not improve after epoch "
                f"{epoch}: {self.epochs_after_best_logloss} / "
                f"{self.stop_after_best_logloss_epochs} since best "
                f"epoch {self.best_epoch_logloss_epoch}"
            )

        if epoch >= self.save_each_epoch_from:
            self._save_step_checkpoint(
                total_step,
                epoch=epoch,
                metrics={
                    ...
                    "is_best_logloss_so_far": is_new_best_logloss,
                    "best_logloss_so_far": self.best_epoch_logloss,
                    "best_logloss_epoch_so_far": self.best_epoch_logloss_epoch,
                    "epochs_after_best_logloss": self.epochs_after_best_logloss,
                },
            )

        return (
            self.stop_after_best_logloss_epochs > 0
            and self.epochs_after_best_logloss
            >= self.stop_after_best_logloss_epochs
        )
        ...
        for epoch in range(1, self.num_epochs + 1):
            ...
            for step, batch in train_pbar:
                ...
                if self.eval_every_n_steps > 0 and total_step % self.eval_every_n_steps == 0:
                    ...
                    if self.save_each_epoch_from <= 0:
                        self._handle_validation_result(total_step, val_auc, val_logloss)

                    if self.save_each_epoch_from <= 0 and self.early_stopping.early_stop:
            ...
            if self.save_each_epoch_from > 0:
                should_stop = self._handle_epoch_logloss_checkpoint(
                    epoch,
                    total_step,
                    val_auc,
                    val_logloss,
                ...
                if should_stop:
                    logging.info(
                        "Stopping after the third epoch-end checkpoint after "
                        f"best LogLoss epoch {self.best_epoch_logloss_epoch}"
                    )
                    break
            else:
                self._handle_validation_result(total_step, val_auc, val_logloss)
    ...
    def _make_model_input(self, device_batch: Dict[str, Any]) -> ModelInput:
        ...
        seq_raw_lens: Dict[str, torch.Tensor] = {}
        seq_time_buckets: Dict[str, torch.Tensor] = {}
        for domain in seq_domains:
            seq_data[domain] = device_batch[domain]
            seq_lens[domain] = device_batch[f'{domain}_len']
            seq_raw_lens[domain] = device_batch.get(f'{domain}_raw_len', seq_lens[domain])
        ...
        return ModelInput(
            ...
            time_dense_feats=device_batch['time_dense_feats'],
            missing_indicator_feats=device_batch.get(
                'missing_indicator_feats',
                device_batch['user_dense_feats'].new_zeros(device_batch['user_dense_feats'].shape[0], 0)),
            seq_data=seq_data,
            seq_lens=seq_lens,
            seq_raw_lens=seq_raw_lens,
    ...
    def evaluate(self, epoch: Optional[int] = None) -> Tuple[float, float]:
        ...
        probs_gpu = torch.sigmoid(all_logits.float())
        labels_gpu = all_labels

        # Filter NaN predictions (may appear if gradients explode).
        nan_mask = torch.isnan(probs_gpu)
        if bool(nan_mask.any().item()):
            n_nan = int(nan_mask.sum().item())
            logging.warning(f"[Evaluate] {n_nan}/{len(probs_gpu)} predictions are NaN, filtering them out")
            valid_mask = ~nan_mask
            probs_gpu = probs_gpu[valid_mask]
            labels_gpu = labels_gpu[valid_mask]
            all_logits = all_logits[valid_mask]

        probs = probs_gpu.cpu().numpy()
        ...
        if len(all_logits) > 0:
            logloss = F.binary_cross_entropy_with_logits(
                all_logits.float(), labels_gpu.float()).item()
        else:
            logloss = float('inf')
