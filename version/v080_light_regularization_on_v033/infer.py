...
_FALLBACK_MODEL_CFG = {
    ...
    'time_dense_dim': TIME_DENSE_DIM,
    'use_seq_len_features': True,
    'use_missing_features': True,
    'missing_indicator_dim': MISSING_INDICATOR_DIM,
    ...
    'user_ns_tokens': 0,
    'item_ns_tokens': 0,
    'use_time_seq_token': False,
    'ns_dense_dropout': 0.0,
    'aux_projector_dropout': 0.0,
    'output_projector_dropout': 0.0,
}
...
def resolve_model_cfg(train_config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for key in _MODEL_CFG_KEYS:
        ...
        if key == 'time_dense_dim':
            if 'time_dense_dim' in train_config:
                cfg[key] = train_config['time_dense_dim']
            elif 'use_exposure_time_features' in train_config:
                cfg[key] = TIME_DENSE_DIM if train_config['use_exposure_time_features'] else 0
            else:
                cfg[key] = _FALLBACK_MODEL_CFG[key]
                logging.warning(
                    f"train_config missing both 'time_dense_dim' and "
                    f"'use_exposure_time_features', using fallback = {cfg[key]}")
            continue
