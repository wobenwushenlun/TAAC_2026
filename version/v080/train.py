...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('--save_each_epoch_from', type=int, default=0,
                        help='If > 0, keep a separate epoch checkpoint from '
                             'this epoch onward instead of using best-only '
                             'checkpoint replacement.')
    parser.add_argument('--stop_after_best_logloss_epochs', type=int, default=0,
                        help='If > 0 together with --save_each_epoch_from, '
                             'stop after this many epoch-end validations '
                             'without beating the lowest validation logloss.')
    ...
    parser.add_argument('--valid_num_workers', type=int, default=0,
                        help='Number of DataLoader workers for validation '
                             '(0 preserves the original single-process validation loader)')
    ...
    parser.add_argument('--ns_dense_dropout', type=float, default=0.0,
                        help='Light dropout after user/item dense token projectors')
    parser.add_argument('--aux_projector_dropout', type=float, default=0.0,
                        help='Light dropout after seq_len/missing auxiliary projectors')
    parser.add_argument('--output_projector_dropout', type=float, default=0.0,
                        help='Light dropout after final Q output projection')
    ...
    parser.add_argument('--use_exposure_time_features', action='store_true', default=True,
                        help='Enable cyclical exposure-time dense features')
    parser.add_argument('--no_exposure_time_features',
                        dest='use_exposure_time_features', action='store_false',
                        help='Disable cyclical exposure-time dense features')
    parser.add_argument('--use_seq_len_features', action='store_true', default=False,
                        help='Append per-domain sequence length summary features')
    parser.add_argument('--use_missing_features', action='store_true', default=False,
                        help='Append selected individual and grouped missing indicators')
    parser.add_argument('--use_time_seq_token', action='store_true', default=False,
                        help='Move exposure-time dense features from output residual to prepended sequence tokens')
    ...
    parser.add_argument('--amp_dtype', type=str, default='none',
                        ...
                        help='CUDA mixed precision for faster training. '
                             'bf16 is recommended on Ampere/Ada GPUs because it does not need GradScaler.')
    ...
    args.missing_indicator_dim = MISSING_INDICATOR_DIM
...
def main() -> None:
    ...
    model_args = {
        ...
        "time_dense_dim": TIME_DENSE_DIM if args.use_exposure_time_features else 0,
        "use_seq_len_features": args.use_seq_len_features,
        "use_missing_features": args.use_missing_features,
        "missing_indicator_dim": args.missing_indicator_dim if args.use_missing_features else 0,
        ...
        "use_time_seq_token": args.use_time_seq_token,
        "ns_dense_dropout": args.ns_dense_dropout,
        "aux_projector_dropout": args.aux_projector_dropout,
        "output_projector_dropout": args.output_projector_dropout,
    }

    model = PCVRHyFormer(**model_args).to(args.device)
    args.time_dense_dim = model_args["time_dense_dim"]
    ...
    trainer = PCVRHyFormerRankingTrainer(
        ...
        save_each_epoch_from=args.save_each_epoch_from,
        stop_after_best_logloss_epochs=args.stop_after_best_logloss_epochs,
    )
