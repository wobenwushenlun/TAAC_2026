...
class ModelInput(NamedTuple):
    ...
    time_dense_feats: torch.Tensor
    missing_indicator_feats: torch.Tensor
    seq_data: dict        # {domain: tensor [B, S, L]}
    seq_lens: dict        # {domain: tensor [B]}
    seq_raw_lens: dict    # {domain: tensor [B]}
...
class HeadRMSNorm(nn.Module):
    """RMSNorm on the attention head dimension for Q/K stabilization."""
...
class RoPEMultiheadAttention(nn.Module):
    ...
    def __init__(
        ...
        self.q_norm = HeadRMSNorm(self.head_dim) if use_qk_norm else nn.Identity()
        self.k_norm = HeadRMSNorm(self.head_dim) if use_qk_norm else nn.Identity()
...
class PCVRHyFormer(nn.Module):
    ...
    def __init__(
        ...
        time_dense_dim: int = 0,
        use_seq_len_features: bool = False,
        use_missing_features: bool = False,
        missing_indicator_dim: int = 0,
        ...
        use_time_seq_token: bool = False,
        ns_dense_dropout: float = 0.0,
        aux_projector_dropout: float = 0.0,
        output_projector_dropout: float = 0.0,
    ) -> None:
        super().__init__()
        ...
        self.use_seq_len_features = use_seq_len_features
        ...
        self.use_time_seq_token = bool(use_time_seq_token)
        self.ns_dense_dropout_rate = float(ns_dense_dropout)
        self.aux_projector_dropout_rate = float(aux_projector_dropout)
        self.output_projector_dropout_rate = float(output_projector_dropout)
        ...
        if self.has_user_dense:
            ...
            self.user_dense_proj_dropout = nn.Dropout(ns_dense_dropout)
        ...
        if self.has_item_dense:
            ...
            self.item_dense_proj_dropout = nn.Dropout(ns_dense_dropout)

        self.has_time_dense = time_dense_dim > 0
        if self.has_time_dense:
            self.time_dense_proj = nn.Sequential(
                nn.Linear(time_dense_dim, d_model),
                nn.LayerNorm(d_model),
            )
            self.time_dense_proj_dropout = nn.Dropout(aux_projector_dropout)

        if self.use_seq_len_features:
            self.seq_len_feature_dim = self.num_sequences * 4
            self.seq_len_proj = nn.Sequential(
                nn.Linear(self.seq_len_feature_dim, d_model),
                nn.LayerNorm(d_model),
                nn.SiLU(),
            )
            self.seq_len_proj_dropout = nn.Dropout(aux_projector_dropout)
        else:
            self.seq_len_feature_dim = 0

        if self.use_missing_features and self.missing_indicator_dim > 0:
            self.missing_proj = nn.Sequential(
                nn.Linear(self.missing_indicator_dim, d_model),
                nn.LayerNorm(d_model),
                nn.SiLU(),
            )
            self.missing_proj_dropout = nn.Dropout(aux_projector_dropout)
        else:
            self.missing_indicator_dim = 0
        ...
        self.output_proj_dropout = nn.Dropout(output_projector_dropout)
    ...
    def _seq_len_features(self, inputs: ModelInput) -> torch.Tensor:
        """Build compact per-domain length features for the classifier."""
        feats = []
        for domain in self.seq_domains:
            clipped_len = inputs.seq_lens[domain].float()
            raw_len = inputs.seq_raw_lens.get(domain, inputs.seq_lens[domain]).float()
            max_len = float(inputs.seq_data[domain].shape[2])
            clipped_ratio = clipped_len / max_len
            raw_log = torch.log1p(raw_len) / math.log1p(max_len * 8.0)
            is_truncated = (raw_len > clipped_len).float()
            is_empty = (raw_len <= 0).float()
            feats.extend([clipped_ratio, raw_log, is_truncated, is_empty])
        return torch.stack(feats, dim=1)

    def _append_aux_features(self, output: torch.Tensor, inputs: ModelInput) -> torch.Tensor:
        parts = [output]
        if self.use_seq_len_features:
            parts.append(self.seq_len_proj_dropout(
                self.seq_len_proj(self._seq_len_features(inputs))))
        if self.use_missing_features and self.missing_indicator_dim > 0:
            missing_feats = inputs.missing_indicator_feats.float()
            if missing_feats.shape[1] != self.missing_indicator_dim:
                missing_feats = missing_feats.new_zeros(missing_feats.shape[0], self.missing_indicator_dim)
            parts.append(self.missing_proj_dropout(self.missing_proj(missing_feats)))
        return torch.cat(parts, dim=-1) if len(parts) > 1 else output

    def _prepend_time_seq_token(
        self,
        seq_tokens_list: list,
        seq_masks_list: list,
        inputs: ModelInput,
    ) -> tuple:
        if not (self.has_time_dense and self.use_time_seq_token):
            return seq_tokens_list, seq_masks_list
        # Keep the proven v033 exposure-time prefix stable; regularize only
        # the weaker auxiliary/output paths in this version.
        time_token = F.silu(self.time_dense_proj(inputs.time_dense_feats)).unsqueeze(1)
        new_tokens = []
        new_masks = []
        for tokens, mask in zip(seq_tokens_list, seq_masks_list):
            new_tokens.append(torch.cat([time_token, tokens], dim=1))
            prefix_mask = torch.zeros(mask.shape[0], 1, dtype=torch.bool, device=mask.device)
            new_masks.append(torch.cat([prefix_mask, mask], dim=1))
    ...
    def _run_multi_seq_blocks(
        ...
        output = self.output_proj_dropout(self.output_proj(output))  # (B, D)
    ...
    def forward(self, inputs: ModelInput) -> torch.Tensor:
        ...
        ns_parts = [user_ns]
        if self.has_user_dense:
            user_dense_tok = self.user_dense_proj_dropout(
                F.silu(self.user_dense_proj(inputs.user_dense_feats))).unsqueeze(1)  # (B, 1, D)
            ns_parts.append(user_dense_tok)
        ns_parts.append(item_ns)
        if self.has_item_dense:
            item_dense_tok = self.item_dense_proj_dropout(
                F.silu(self.item_dense_proj(inputs.item_dense_feats))).unsqueeze(1)  # (B, 1, D)
        ...
        seq_tokens_list, seq_masks_list = self._prepend_time_seq_token(
            seq_tokens_list, seq_masks_list, inputs)
        ...
        if self.has_time_dense and not self.use_time_seq_token:
            output = output + self.time_dense_proj_dropout(
                F.silu(self.time_dense_proj(inputs.time_dense_feats)))
        output = self._append_aux_features(output, inputs)
