...
HASH_SEQ_FEATURE_SPECS = {
    ('seq_c', 29): 200000,
    ('seq_c', 47): 200000,
}

MISSING_INDICATOR_SPECS = [
    ('user_int_feats_3', 'user_int_feats_3'),
    ('user_int_feats_4', 'user_int_feats_4'),
    ('user_int_feats_48', 'user_int_feats_48'),
    ('user_int_feats_49', 'user_int_feats_49'),
    ('user_int_feats_50', 'user_int_feats_50'),
    ('item_int_feats_8', 'item_int_feats_8'),
    ('item_int_feats_83', 'item_int_feats_83'),
    ('item_int_feats_84', 'item_int_feats_84'),
    ('item_int_feats_85', 'item_int_feats_85'),
    ('user_dense_feats_61', 'user_dense_feats_61'),
]

MISSING_GROUP_SPECS = [
    ('fid91', ['user_int_feats_91', 'user_dense_feats_91']),
    ('item_int_83_85', ['item_int_feats_83', 'item_int_feats_84', 'item_int_feats_85']),
    (
        'user_int_100_103_109_99',
        [
            'user_int_feats_99',
            'user_int_feats_100',
            'user_int_feats_101',
...
MISSING_INDICATOR_DIM = len(MISSING_INDICATOR_SPECS) + 3 * len(MISSING_GROUP_SPECS)

DENSE_LOG_CLIP_SPECS = {
    # Caps come from online full-data EDA p99 statistics. Values are clipped,
    # log1p transformed, then divided by log1p(cap), so missing-filled zeros
    # stay at 0 and large count-like values land in a stable [0, 1] range.
    62: 2030742.0,
    63: 1995563.0,
    64: 2039857.0,
    65: 2371572.0,
...
class PCVRParquetDataset(IterableDataset):
    ...
    def __init__(
        ...
        row_group_items: Optional[List[Tuple[str, int, int]]] = None,
        ...
        if row_group_items is not None:
            self._rg_list = list(row_group_items)
        else:
            self._rg_list = []
        ...
        if row_group_range is not None and row_group_items is None:
        ...
        self._buf_time_dense = np.zeros((B, TIME_DENSE_DIM), dtype=np.float32)
        self._buf_seq = {}
        self._buf_seq_tb = {}
        self._buf_seq_lens = {}
        self._buf_seq_raw_lens = {}
        for domain in self.seq_domains:
            ...
            self._buf_seq_raw_lens[domain] = np.zeros(B, dtype=np.int64)
        self.missing_indicator_dim = MISSING_INDICATOR_DIM
        self._buf_missing_indicators = np.zeros((B, self.missing_indicator_dim), dtype=np.float32)
        ...
        self._missing_indicator_plan = [
            self._col_idx.get(col_name) for _, col_name in MISSING_INDICATOR_SPECS
        ]
        self._missing_group_plan = [
            [self._col_idx.get(col_name) for col_name in col_names]
            for _, col_names in MISSING_GROUP_SPECS
        ]

        # Sequence column plan: {domain: ([(col_idx, feat_slot, vocab_size, fid, hash_bucket), ...], ts_col_idx)}
        self._seq_plan = {}
        for domain in self.seq_domains:
            ...
            for slot, fid in enumerate(sideinfo_fids):
                ...
                hash_bucket = HASH_SEQ_FEATURE_SPECS.get((domain, fid), 0)
                side_plan.append((ci, slot, vs, fid, hash_bucket))
    ...
    def _load_schema(self, schema_path: str, seq_max_lens: Dict[str, int]) -> None:
        ...
        for domain in self.seq_domains:
            ...
            self.seq_vocab_sizes[domain] = {
                fid: HASH_SEQ_FEATURE_SPECS.get((domain, fid), vs)
    ...
    @staticmethod
    def _build_time_dense(timestamps: "npt.NDArray[np.int64]") -> "npt.NDArray[np.float32]":
        """Build cyclical exposure-time features from Unix timestamps.

        Absolute timestamps are intentionally not exposed to the model. The
        four features are sin/cos(hour_of_day) and sin/cos(day_of_week).
        """
        ts = timestamps.astype(np.int64)
        seconds_in_day = 24 * 3600
        seconds_in_week = 7 * seconds_in_day
        day_phase = (ts % seconds_in_day).astype(np.float32) / float(seconds_in_day)
        week_phase = (ts % seconds_in_week).astype(np.float32) / float(seconds_in_week)
        two_pi = np.float32(2.0 * np.pi)
        return np.stack([
            np.sin(two_pi * day_phase),
            np.cos(two_pi * day_phase),
            np.sin(two_pi * week_phase),
            np.cos(two_pi * week_phase),
    ...
    def _transform_user_dense_column(
        self,
        fid: int,
        values: "npt.NDArray[np.float32]",
    ) -> "npt.NDArray[np.float32]":
        cap = DENSE_LOG_CLIP_SPECS.get(fid)
        if cap is None:
            return values
        clipped = np.clip(values, 0.0, cap)
        return (np.log1p(clipped) / np.log1p(cap)).astype(np.float32)

    def _scalar_missing_mask(self, arrow_col: "pa.Array", B: int) -> "npt.NDArray[np.bool_]":
        arr = arrow_col.fill_null(0).to_numpy(zero_copy_only=False)
        return np.asarray(arr <= 0, dtype=bool)

    def _list_missing_mask(self, arrow_col: "pa.ListArray", B: int) -> "npt.NDArray[np.bool_]":
        offsets = arrow_col.offsets.to_numpy()
        missing = (offsets[1:B + 1] - offsets[:B]) <= 0
        if arrow_col.null_count:
            nulls = np.asarray(arrow_col.is_null().to_numpy(zero_copy_only=False), dtype=bool)
            missing = missing | nulls
        return missing

    def _column_missing_mask(
        self, batch: "pa.RecordBatch", ci: Optional[int], B: int
    ) -> "npt.NDArray[np.bool_]":
        if ci is None:
            return np.ones(B, dtype=bool)
        col = batch.column(ci)
        if pa.types.is_list(col.type) or pa.types.is_large_list(col.type):
            return self._list_missing_mask(col, B)
        return self._scalar_missing_mask(col, B)

    def _build_missing_indicators(
        self, batch: "pa.RecordBatch", B: int
    ) -> "npt.NDArray[np.float32]":
        missing_indicators = self._buf_missing_indicators[:B]
        missing_indicators[:] = 0
        out_idx = 0
        for ci in self._missing_indicator_plan:
            missing_indicators[:, out_idx] = self._column_missing_mask(batch, ci, B).astype(np.float32)
            out_idx += 1

        for group_plan in self._missing_group_plan:
            masks = [self._column_missing_mask(batch, ci, B) for ci in group_plan]
            if masks:
                stacked = np.stack(masks, axis=1)
                any_missing = stacked.any(axis=1)
                all_missing = stacked.all(axis=1)
            else:
                any_missing = np.ones(B, dtype=bool)
                all_missing = np.ones(B, dtype=bool)
            complete = ~any_missing
            partial_missing = any_missing & ~all_missing
            missing_indicators[:, out_idx] = complete.astype(np.float32)
            missing_indicators[:, out_idx + 1] = partial_missing.astype(np.float32)
            missing_indicators[:, out_idx + 2] = all_missing.astype(np.float32)
            out_idx += 3
    ...
    def _convert_batch(self, batch: "pa.RecordBatch") -> Dict[str, Any]:
        ...
        time_dense = self._buf_time_dense[:B]
        time_dense[:] = self._build_time_dense(timestamps)
        if self.is_training:
        ...
        for ci, dim, offset, fid in self._user_dense_plan:
            ...
            padded = self._transform_user_dense_column(fid, padded)
            user_dense[:, offset:offset + dim] = padded

        missing_indicators = self._build_missing_indicators(batch, B)
        ...
        result = {
            ...
            'time_dense_feats': torch.from_numpy(time_dense.copy()),
            'missing_indicator_feats': torch.from_numpy(missing_indicators.copy()),
        ...
        for domain in self.seq_domains:
            ...
            lengths[:] = 0
            raw_lengths = self._buf_seq_raw_lens[domain][:B]
            raw_lengths[:] = 0
            ...
            col_data = []
            for ci, slot, vs, fid, hash_bucket in side_plan:
                col = batch.column(ci)
                col_data.append((col.offsets.to_numpy(), col.values.to_numpy(), vs, ci, hash_bucket))

            for c, (offs, vals, vs, ci, hash_bucket) in enumerate(col_data):
                for i in range(B):
                    s = int(offs[i])
                    e = int(offs[i + 1])
                    rl = e - s
                    ...
                    src = vals[s:s + ul]
                    if hash_bucket:
                        mapped = np.zeros_like(src)
                        pos = src > 0
                        mapped[pos] = ((src[pos] - 1) % hash_bucket) + 1
                        src = mapped
                    out[i, c, :ul] = src
                    if ul > lengths[i]:
                        lengths[i] = ul
            ...
            for c, (_, _, vs, ci, _) in enumerate(col_data):
                slice_c = out[:, c, :]
                if vs > 0:
            ...
            result[f'{domain}_raw_len'] = torch.from_numpy(raw_lengths.copy())
...
def _get_row_group_timestamp_range(
    pf: pq.ParquetFile,
    file_path: str,
    rg_idx: int,
    timestamp_col_idx: int,
) -> Tuple[int, int]:
    """Return ``(min_timestamp, max_timestamp)`` for one Row Group."""
    col_meta = pf.metadata.row_group(rg_idx).column(timestamp_col_idx)
    stats = col_meta.statistics
    if stats is not None and stats.has_min_max:
        return int(stats.min), int(stats.max)

    ts = pf.read_row_group(rg_idx, columns=['timestamp']).column('timestamp')
    arr = ts.to_numpy(zero_copy_only=False).astype(np.int64)
    if arr.size == 0:
        logging.warning(f"Empty timestamp column in {file_path} row_group={rg_idx}")
        return 0, 0
...
def get_pcvr_data(
    ...
    **kwargs: Any,
    ...
    logging.info(f"Parquet train: {train_rows} rows, valid: {valid_rows} rows, "
                 ...
                 f"train_workers={num_workers}, valid_workers={valid_num_workers}")
